...
function syncInstructionPrompt(){
  const instructionsOpen=!instructions.classList.contains("hidden");
  helpButton.classList.toggle("instructions-open",instructionsOpen);
  helpButton.setAttribute("aria-label",instructionsOpen?"Hide instructions":"Show instructions");
  helpButton.title=instructionsOpen?"Close instructions":"Show instructions";
}

new MutationObserver(syncInstructionPrompt).observe(instructions,{attributes:true,attributeFilter:["class"]});
syncInstructionPrompt();

restartButton.addEventListener("click",restartCourse);
recoverButton.addEventListener("click",recoverBall);
playAgainButton.addEventListener("click",restartCourse);
helpButton.addEventListener("click",()=>{instructions.classList.toggle("hidden");playTone(380,.045,"sine",.028)});
soundButton.addEventListener("click",()=>{state.audioEnabled=!state.audioEnabled;soundButton.textContent=state.audioEnabled?"♪":"×";soundButton.setAttribute("aria-label",state.audioEnabled?"Mute sound":"Enable sound");if(state.audioEnabled)playTone(620,.06,"triangle",.04)});
...
addEventListener("orientationchange",resizeCanvas,{passive:true});